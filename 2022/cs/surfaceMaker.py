...
#Read in geometry
a,b,c = np.loadtxt('EX.txt',usecols=(0,1,2),unpack=True)
#Read in frequency 1
...
xxx = np.arange(-10,12,2)
yyy = np.arange(-10,12,2)
...
for i in range(0,np.size(xxx)):
    for j in range(0,np.size(yyy)):
        ...
        fn = 'c'+str(fn1)+'-'+str(fn2)+'/s_sp_'+str(xxx[i])+'_'+str(yyy[j])+'.txt'
        gm += '\n'
        T = serv+gm
        text_file = open(fn,"w")
        text_file.write(T)
...
G = gauss+fl
fn = 'c'+str(fn1)+'-'+str(fn2)+'/gauss'+str(fn1)+str(fn2)
