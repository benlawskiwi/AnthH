...
fr = []
for i in range(0,23):
    a,b,c = np.loadtxt('h6-EX.txt',unpack=True,skiprows=2+32*i,usecols=(2,3,4),max_rows=1)
    fr.append(a)
    fr.append(b)
    fr.append(c)
...
sym = []
for i in range(0,23):
    a,b,c = np.loadtxt('h6-EX.txt',unpack=True,skiprows=1+32*i,usecols=(0,1,2),max_rows=1,dtype='str')
    sym.append(a)
    sym.append(b)
    sym.append(c)
...
for j in range(0,25):
    for i in range(0,23):
        a,b,c = np.loadtxt('h6-EX.txt',unpack=True,skiprows=7+32*i+j,usecols=(4,7,10),max_rows=1)
        x1.append(a)
        x1.append(b)
        x1.append(c)
    ar = np.array(x1)
...
for j in range(0,25):
    for i in range(0,23):
        a,b,c = np.loadtxt('h6-EX.txt',unpack=True,skiprows=7+32*i+j,usecols=(2,5,8),max_rows=1)
        x1.append(a)
        x1.append(b)
        x1.append(c)
    ar = np.array(x1)
...
for j in range(0,25):
    for i in range(0,23):
        a,b,c = np.loadtxt('h6-EX.txt',unpack=True,skiprows=7+32*i+j,usecols=(3,6,9),max_rows=1)
        x1.append(a)
        x1.append(b)
        x1.append(c)
    ar = np.array(x1)
...
plt.plot((0,0),(0,0),'C0',label="A'")
plt.plot((0,0),(0,0),'C2',label='A"')
...
print(final)
np.savetxt('h6-ex-Matrix.txt',final,fmt='%1.0f    %1.2f       %1.2f')
